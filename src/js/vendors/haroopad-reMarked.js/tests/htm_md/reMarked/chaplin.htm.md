...
* Then you’re able to access _http://chaplin.moviepilot.com/_ in your browser and log in with Facebook.
...
In Backbone’s concept, there are models and views, but no controllers. Backbone’s router maps routes to its _own methods_, so it’s serves two purposes. Our `Router` is just a router, it maps URLs to _separate controllers_, in particular controller actions. Just like Backbone’s standard router, we’re using an instance of `Backbone.History` in the background.
